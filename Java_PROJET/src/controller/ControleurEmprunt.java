...
public class ControleurEmprunt {
    ...
    private JPanel createEmpruntInputPanel(Emprunt emprunt) {
        ...
        panel.add(new JLabel("ID du Livre :"));
        panel.add(livreIdField);
        panel.add(new JLabel("Nombre de jours pour l'emprunt :"));
        panel.add(nombreJoursField);

        panel.putClientProperty("utilisateurComboBox", utilisateurComboBox);
    ...
    private Emprunt extractEmpruntFromPanel(JPanel panel) throws NumberFormatException, IllegalArgumentException {
        JComboBox<String> utilisateurComboBox = (JComboBox<String>) panel.getClientProperty("utilisateurComboBox");
        ...
        LocalDate dateRetourPrevu = dateEmprunt.plusDays(nombreJours);

        return new Emprunt(utilisateurId, livreId, dateEmprunt.format(DATE_FORMATTER), dateRetourPrevu.format(DATE_FORMATTER));
    }
    ...
    private boolean isLivreDisponible(int livreId) throws LivreNotFoundException {
        Livre livre = livreModel.rechercherParID(livreId);
